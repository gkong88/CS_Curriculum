...
public class BPlusTree implements Closeable {
    ...
    public Iterator<RecordId> scanAll(BaseTransaction transaction) {
        DataBox key = root.getLeftmostLeaf(transaction).getKeys().get(0);
        return scanGreaterEqual(transaction, key);
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(BaseTransaction transaction, DataBox key) {
        typecheck(key);
        ArrayList<RecordId> results = new ArrayList<RecordId>();
        LeafNode leaf = root.get(transaction, key);
        Iterator<RecordId> leafRecords = leaf.scanGreaterEqual(key);
        while (leafRecords.hasNext()) {
            results.add(leafRecords.next());
        }
        while (leaf.getRightSibling(transaction).isPresent()) {
            leaf =  leaf.getRightSibling(transaction).get();
            leafRecords = leaf.scanAll();
            while (leafRecords.hasNext()) {
                results.add(leafRecords.next());
            }
        }
    ...
    public void put(BaseTransaction transaction, DataBox key, RecordId rid) throws BPlusTreeException {
        typecheck(key);
        Optional<Pair<DataBox, Integer>> splitKey = root.put(transaction, key, rid);
    ...
    public void bulkLoad(BaseTransaction transaction, Iterator<Pair<DataBox, RecordId>> data,
        ...
        if (root.getLeftmostLeaf(transaction).getKeys().size() > 0) {
        ...
        while (data.hasNext()) {
            splitKey = root.bulkLoad(transaction, data, fillFactor);
            if (splitKey.isPresent()) {
                    root = new InnerNode(metadata, new ArrayList<DataBox>(Arrays.asList(splitKey.get().getFirst())), new ArrayList<Integer>(Arrays.asList(root.getPage().getPageNum(), splitKey.get().getSecond())), transaction);
                }
        }
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(BaseTransaction transaction, BPlusTree tree) {
//            this.tree = tree;
            iterator = tree.scanAll(transaction);
        }
        @Override
