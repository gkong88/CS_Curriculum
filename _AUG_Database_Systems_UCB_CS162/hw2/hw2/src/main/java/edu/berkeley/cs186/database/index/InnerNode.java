...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(BaseTransaction transaction, DataBox key) {
        ...
        return BPlusNode.fromBytes(transaction, metadata, child).get(transaction, key);
    }
    ...
    public LeafNode getLeftmostLeaf(BaseTransaction transaction) {
        ...
        return BPlusNode.fromBytes(transaction, metadata, child).getLeftmostLeaf(transaction);
    }
    ...
    throws BPlusTreeException {
        ...
        Optional<Pair<DataBox, Integer>> overflow = BPlusNode.fromBytes(transaction, metadata, children.get(child)).put(transaction, key, rid);
        if (overflow.isPresent()) {
            DataBox overflowKey = overflow.get().getFirst();
            int overflowPageNum = overflow.get().getSecond();
            keys.add(child, overflowKey);
            children.add(child + 1, overflowPageNum);
            if (keys.size() > metadata.getOrder() * 2) {
                int splitIndex = keys.size() / 2;
                DataBox splitKey = keys.get(splitIndex);
                List<DataBox> splitKeys = new ArrayList<DataBox>(keys.subList(splitIndex + 1, keys.size()));
                List<Integer> splitChildren = new ArrayList<Integer>(children.subList(splitIndex + 1, children.size()));
                InnerNode splitSibling = new InnerNode(metadata, splitKeys, splitChildren, transaction);
                keys = new ArrayList<DataBox>(keys.subList(0, splitIndex));
                children = new ArrayList<Integer>(children.subList(0, splitIndex + 1));
                sync(transaction);
                return Optional.of(new Pair<DataBox, Integer>(splitKey, splitSibling.getPage().getPageNum()));
            } else {
                sync(transaction);
    ...
    public Optional<Pair<DataBox, Integer>> bulkLoad(BaseTransaction transaction,
            ...
            float fillFactor)
    throws BPlusTreeException {
        int keyCapacity = (int) Math.ceil(metadata.getOrder() * 2);
        Optional<Pair<DataBox, Integer>> splitKey;
        // precondition: this has one key and two children, a fresh inner node
        while (data.hasNext()) {
            splitKey = getChild(transaction, children.size() - 1).bulkLoad(transaction, data, fillFactor);
            if (splitKey.isPresent()) {
                if (keys.size() < keyCapacity) {
                    keys.add(splitKey.get().getFirst());
                    children.add(splitKey.get().getSecond());
                }
                else {
                    sync(transaction);
                    return Optional.of(new Pair<DataBox, Integer>(splitKey.get().getFirst(), splitKey.get().getSecond()));                }
            }
        }
        sync(transaction);
        return Optional.empty();
    ...
    public void remove(BaseTransaction transaction, DataBox key) {
        ...
        BPlusNode.fromBytes(transaction, metadata, children.get(child_index)).remove(transaction, key);
        sync(transaction);
    }
    ...
    public static InnerNode fromBytes(BaseTransaction transaction, BPlusTreeMetadata metadata,
                                      int pageNum) {
        Page page = metadata.getAllocator().fetchPage(transaction, pageNum); // give me a page
//        Buffer buf = page.getBuffer(transaction); //from the page,
        byte[] Bytes = metadata.getAllocator().fetchPage(transaction, pageNum).readBytes(transaction);
        ByteArrayInputStream bInput = new ByteArrayInputStream(Bytes);
        assert(bInput.read() == (byte) 0); // verify is an inner node
        byte numKeysByteArray[] = new byte[4]; // allocate array
        bInput.read(numKeysByteArray, 0, 4); // read data into my buffer, up tos ize of array
        int n = ByteBuffer.wrap(numKeysByteArray).getInt(); // number of entries
        DataBox key;
        int child;
        byte keyByteArray[] = new byte[metadata.getKeySchema().getSizeInBytes()];
        byte childByteArray[] = new byte[4];
        List<DataBox> keys = new ArrayList<>();
        List<Integer> children = new ArrayList<>();
        for (int i = 0; i < n; ++i) {
            bInput.read(keyByteArray, 0, metadata.getKeySchema().getSizeInBytes());
            key = DataBox.fromBytes(ByteBuffer.wrap(keyByteArray), metadata.getKeySchema());
            keys.add(key);
        }
        for (int i = 0; i < n + 1; ++i) {
            bInput.read(childByteArray, 0, 4);
            child = ByteBuffer.wrap(childByteArray).getInt();
            children.add(child);
        }
