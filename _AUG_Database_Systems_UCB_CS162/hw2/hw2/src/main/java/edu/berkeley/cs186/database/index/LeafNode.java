...
class LeafNode extends BPlusNode {
    ...
    throws BPlusTreeException {
        ...
        if (keys.size() > metadata.getOrder() * 2) {
            ...
            LeafNode splitLeaf = new LeafNode(metadata, new ArrayList<DataBox>(keys.subList(splitIndex, keys.size())), new ArrayList<RecordId>(rids.subList(splitIndex, keys.size())), rightSibling, transaction);
            rightSibling = Optional.of(splitLeaf.getPage().getPageNum());
            keys = new ArrayList<DataBox>(keys.subList(0, splitIndex));
            rids = new ArrayList<RecordId>(rids.subList(0, splitIndex));

            sync(transaction);
            System.out.println("splitting on" + splitKey.toString());
            return Optional.of(new Pair<>(splitKey, splitLeaf.getPage().getPageNum()));
        }
        else {
            sync(transaction);
    ...
    public Optional<Pair<DataBox, Integer>> bulkLoad(BaseTransaction transaction,
            ...
            float fillFactor)
    throws BPlusTreeException {
        int leafCapacity = (int) Math.ceil(fillFactor * metadata.getOrder() * 2);
        while (keys.size() < leafCapacity && data.hasNext()) { //leaf has capacity, still more records to add
            //"add stuff here";
            Pair<DataBox, RecordId> nextItem = data.next();
            DataBox nextKey = nextItem.getFirst();
            RecordId nextRecordId = nextItem.getSecond();
            keys.add(nextKey);
            rids.add(nextRecordId);
        }
        if (data.hasNext()) { // more data, and we reached our capacity
            Pair<DataBox, RecordId> splitData = data.next();
            DataBox splitKey = splitData.getFirst();
            RecordId splitRID = splitData.getSecond();
            LeafNode splitLeaf = new LeafNode(metadata, new ArrayList<DataBox>(Arrays.asList(splitKey)), new ArrayList<RecordId>(Arrays.asList(splitRID)), Optional.empty(), transaction);
            rightSibling = Optional.of(splitLeaf.getPage().getPageNum());
            ...
            sync(transaction);
            return Optional.empty(); // success! inserted all of data
        }
    }
    ...
    public static LeafNode fromBytes(BaseTransaction transaction, BPlusTreeMetadata metadata,
        ...
        assert(bInput.read() == 1); // assert page is encoded as a leaf node.
        byte[] rightSiblingByteArray = new byte[4];
        bInput.read(rightSiblingByteArray, 0, 4);
//        int rightSiblingInt = ByteBuffer.wrap(rightSiblingByteArray).getInt();
        int rightSiblingInt = ByteBuffer.wrap(rightSiblingByteArray).getInt();
        ...
        byte[] numKeysByteArray = new byte[4];
        bInput.read(numKeysByteArray, 0, 4);
        int numKeys = ByteBuffer.wrap(numKeysByteArray).getInt();
//        List<DataBox> keys = new ArrayList<DataBox>(numKeys);
//        List<RecordID> rids = new ArrayList<RecordID>(numKeys);
        List<DataBox> keys = new LinkedList<DataBox>();
        List<RecordId> rids = new LinkedList<RecordId>();
        int keySize = metadata.getKeySchema().getSizeInBytes();
        int ridSize = RecordId.getSizeInBytes();
        byte[] keyRidByteArray = new byte[keySize + ridSize];
        Type keyType = metadata.getKeySchema();
        DataBox key;
        int rid_pageNum;
        short rid_entryNum;
        for (int i = 0; i < numKeys; i++) {
            bInput.read(keyRidByteArray, 0, keySize + ridSize);
            key = DataBox.fromBytes(ByteBuffer.wrap(keyRidByteArray, 0, keySize), keyType);
            rid_pageNum = ByteBuffer.wrap(keyRidByteArray, keySize, Integer.BYTES).getInt();
            rid_entryNum = ByteBuffer.wrap(keyRidByteArray, keySize + Integer.BYTES, Short.BYTES).getShort();
            keys.add(key);
            rids.add(new RecordId(rid_pageNum, rid_entryNum));
        }
